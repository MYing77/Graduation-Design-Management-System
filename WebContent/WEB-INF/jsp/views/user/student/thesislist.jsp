						...
						<ul class="nav nav-drawer">
							...
							<li class="nav-item nav-item-has-subnav active open"><a
								href="javascript:void(0)"><i class="mdi mdi-file-document"></i>
									论文管理</a>
								<ul class="nav nav-subnav">
									<li class="active"><a href="${pageContext.request.contextPath }/student/thesislist.action">学生论文 (在线提交)</a></li>
								</ul></li>
							<li class="nav-item nav-item-has-subnav"><a
				...
				<nav class="navbar navbar-default">
					<div class="topbar">
						...
						<div class="topbar-left">
							...
							<span class="navbar-page-title"> 提交论文 </span>
						</div>
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-toolbar clearfix">
								<form id="form_query"
									action="${pageContext.request.contextPath }/student/thesislist.action"
							...
							<c:if test="${i eq 1 && i2 eq 0}">
							    ...
							    <input id="fType" type="text" name="fType" hidden="hidden" value="论文">
								...
								<div class="table-responsive">
									<table class="table table-bordered" id="tbodyID">
										...
										<tbody>
											<c:forEach var="thesis" items="${pageInfo.list }"
												varStatus="state">
												<tr>
													<td>${ state.index + 1}</td>
													<td>${thesis.titlName}</td>
													<td>${thesis.batch}</td>
													<td>
													<button style="border:0;background: none;" type="button" data-toggle="tooltip" data-placement="top" title="第一次预览可能需要等待较长时间"><a href="${pageContext.request.contextPath }/file/toPdfProjBook.action?num1=${thesis.fId }&num2=${thesis.tId }">${thesis.fName }</a></button>
													</td>
													<td><fmt:formatDate value="${thesis.uploadDatetime }" pattern="yyyy-MM-dd hh:mm:ss"></fmt:formatDate></td>
													<td>${thesis.tName }</td>
													<td><span <c:if test="${thesis.agree == '不通过'}">class="label label-danger"</c:if> <c:if test="${thesis.agree == '通过'}">class="label label-success"</c:if> <c:if test="${thesis.agree == '待通过'}">class="label label-warning"</c:if>>${thesis.agree }</span></td>
													<td>
													<input data-toggle="modal"
														data-target="#commentsInfoModal" type="button" class="btn btn-primary btn-xs" id="comment"
																	name="search2" value='查看指导教师建议' onclick="getcomment(${thesis.fId})">
													<a href="${pageContext.request.contextPath }/file/filedown.action?fId=${thesis.fId}"><input type="button" class="btn btn-primary btn-xs" id="download"
						...
						<div class="modal-body">
							<form id="selectById_title_form">
										<div class="form-group">
											<label for="message-text" class="control-label">论文建议内容：</label>
