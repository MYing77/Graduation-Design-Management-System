...
<body>
          ...
          <ul class="nav nav-drawer">
            <li class="nav-item nav-item-has-subnav active open">
              ...
              <ul class="nav nav-subnav">
                <c:if test="${i!=0 }"><li><a href="${pageContext.request.contextPath }/teacher/toReplyleader.action">我的答辩组</a></li></c:if>
                <li class="active"> <a href="${pageContext.request.contextPath }/teacher/toreview.action">论文评分</a> </li>
              </ul>
            </li>
				...
				<nav class="navbar navbar-default">
					<div class="topbar">
						...
						<div class="topbar-left">
							...
							<span class="navbar-page-title"> 论文评分 </span>
						</div>
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-toolbar clearfix">
								<form id="form_query"
									action="${pageContext.request.contextPath }/teacher/toreview.action"
										...
										<tbody>
											<c:forEach var="item" items="${pageInfo.list }"
												...
												<tr>
													...
													<td>${item.batch }</td>
													<td>${item.tId} ${item.tName}</td>
													<td>${item.reviewType }</td>
													<td>${item.reviewScore } <c:if test="${item.reviewScore!=0 }"> 已评阅</c:if>	
												    <c:if test="${item.reviewScore==0 }"> 未评阅</c:if>	</td>
													<td>
													<input data-toggle="modal"
														data-target="#commentsInfoModal" type="button" class="btn btn-primary btn-xs" id="comment"
																	name="search2" value='评阅' onclick="getcomment('${item.sId}','${item.memberTId}','${item.reviewScore}','${item.reviewComments }')">		
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<input hidden="hidden" type="text" id="memberTId" name="memberTId">					
										<div class="form-group">
											...
											<p></p>
											<div class="form-group">
											<label for="message-text" class="control-label">评阅教师评分：</label>
											<div class="example-box">
												<input maxlength="3" type="text" class="form-control" name="reviewScore" id="review_Score" ime-mode:disabled" onkeydown="if(event.keyCode==13)event.keyCode=9" onKeyPress="if ((event.keyCode<48 || event.keyCode>57)) event.returnValue=false"/>
											</div>
											</div>
	...
	//评阅教师对论文编辑评语、评分
	    function getcomment(sId,memberTId,reviewScore,reviewComments) {
	    	$("#s_Id").val(sId);
	    	$("#memberTId").val(memberTId);
	    	$("#review_Score").val(reviewScore);
	  ...
	  //保存评阅教师建议和成绩
	    function saveStudentScore() {
		  if($("#review_Score").val()==""){
	    	...
	    	$.post("<%=basePath%>teacher/editReviewScore.action",
	    			$("#studentScore_form").serialize(),
	    			function(data){
	    			        if(data =="OK"){
	    			            alert("保存成功！");
